...
void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, size_t length)
{
  #pragma omp parallel num_threads(NB_THREADS)
  {
     ...
     for (i = istart; i < iend; ++i)
     {
        ...
        {
           memcpy(buffer, myctx.Iv, AES_BLOCKLEN);
...
static int function_encrypt_ctr(const char* xcrypt)
{
    ...
    printf(" Please enter the input file name (to be crypted) : ");
    scanf("%s",nomFichEntr);
    printf(" Please enter the output file name (recommended .bin extension) : ");
    scanf("%s",nomFichSort);
    ...
    long int fileLen = ftell(entrFichPtr);
    ...
    start = omp_get_wtime();///sauvegarder l'instant de début du cryptage
    AES_init_ctx_iv(&ctx, key, iv);
    AES_CTR_xcrypt_buffer(&ctx, bufEntr, fileLen);
    end = omp_get_wtime();///sauvegarder l'instant de fin du cryptage
    elapsed=end-start;
    ...
    resFichPtr=fopen(nomFichSort,"wb");
    if(resFichPtr==NULL)
    {
    ...
    fclose(resFichPtr);
    printf("\n The elapsed time is %f seconds", elapsed);
}
